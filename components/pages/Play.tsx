...
const REST_DAY_OPTIONS: { type: RestDayType; label: string; emoji: string; description: string; color: string; bg: string; border: string }[] = [
...
export default function PlayPage() {
  const { schemas, history, startWorkout, updateActiveWorkout, deleteSchema, isDataLoaded, restDays, addRestDay, removeRestDay } = useData()
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="p-6 max-w-2xl mx-auto space-y-8">
        ...
        {/* Quick rest-day logging — shown when today has no workout and no rest day yet */}
        {(() => {
          ...
          if (hasWorkoutToday) return null; // already trained today, don't show
              ...
              <div className={`flex items-center justify-between px-4 py-3 rounded-xl border ${meta.bg} ${meta.border}`}>
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    {todayRestDay.note && <p className="text-[11px] text-muted-foreground">{todayRestDay.note}</p>}
                  </div>
                </div>
                <button
